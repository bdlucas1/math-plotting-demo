...
Supported now to some degree sufficient for a demo:

* Row[{item, ...}] where items can be graphics or other expr (displayed as text for now)
* Grid[{{item, ...}, ...}]
* Manipulate
* Plot, NumberLinePlot, PolarPlot, etc. - old code, in some cases currently fast enough for Manipulate
...
* Axes -> True or False
* full math formatting using latex and mathjax
* compiled plotting expressions with functions like Sin, operators
  like +, =, +=, procedural constructs like Module, CompoundExpression, For
  that operate efficientlhy on numpy arrays
